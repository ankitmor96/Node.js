...
const server = http.createServer((req,res)=>{  // server variable create and callback function use req/res
    res.write("hello ankit , kem che ? ");   // write response to node in browser localhost 
...
server.listen(port,(err)=>{
    if(err){       // if error condition is false
        return console.log("This is not response");  // stope return false part
    }
    console.log(`This is response to Work server ${port}`);   // else condition is true so execute node.js
});
